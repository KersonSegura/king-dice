...
async function fixHotnessFinal() {
  try {
    ...
    if (notFound.length > 0) {
      console.log(`\n❌ Missing games (${notFound.length}):`);
      notFound.forEach(name => console.log(`  - ${name}`));
      
      // Add missing games
      console.log('\n📝 Adding missing games...');
      for (const gameInfo of notFound) {
        const newGame = await prisma.game.create({
          data: {
            ...
            year: gameInfo.year,
            category: 'hotness',
            hotnessRank: hotnessOrder.findIndex(g => g.name === gameInfo.name) + 1
          }
        });
        
        console.log(`✅ Created: ${gameInfo.name} (ID: ${newGame.id})`);
      }
      
      // Now update the hotnessRank for newly added games
      for (let i = 0; i < hotnessOrder.length; i++) {
        const gameInfo = hotnessOrder[i];
