...
async function main() {
  try {
    console.log('🚀 Iniciando actualización de ratings...');
    
    // Lista de juegos importantes para actualizar
    const importantGameIds = [
      13,      // Catan
      237182,  // Root
    ...
    for (let i = 0; i < importantGameIds.length; i++) {
      const bggId = importantGameIds[i];
      console.log(`🔄 Procesando juego ${i + 1}/${importantGameIds.length} (BGG ID: ${bggId})...`);
      ...
      if (i < importantGameIds.length - 1) {
        await delay(2000);
      }
    }
