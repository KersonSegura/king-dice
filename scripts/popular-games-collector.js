...
class PopularGamesCollector {
  ...
  async collectFromMostPopular(limit = 100) {
    try {
      ...
      const searchTerms = ['strategy', 'family', 'party', 'cooperative', 'deck building', 'worker placement', 'euro', 'ameritrash', 'card game', 'dice game'];
      let allGames = [];
  ...
  async runPopularCollection() {
    try {
      console.log('🚀 STARTING POPULAR GAMES COLLECTION 🚀\n');
      console.log('⏰ Start time:', new Date().toLocaleString());
      console.log('🎯 This will collect popular games from multiple search terms...\n');
      
      // Collect from most popular source
      const results = await this.collectFromMostPopular(100);
      this.totalCollected = results.collected;
      this.totalSkipped = results.skipped;
      this.totalErrors = results.errors;
      
      // Final results
      console.log(`\n🏆 POPULAR GAMES COLLECTION COMPLETED!`);
      ...
      console.error('❌ Error in popular games collection:', error);
    } finally {
...
async function main() {
  try {
    const collector = new PopularGamesCollector();
    await collector.runPopularCollection();
  } catch (error) {
