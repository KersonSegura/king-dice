  ...
  try {
    ...
    try {
      ...
      gamesToFind.forEach((gameName) => {
        ...
        if (matchedGame) {
          if (!matchedGame.id) {
            console.warn(`⚠️ Matched game "${gameName}" but it has no ID`);
            missingGames.push(gameName);
          } else if (matchedGameIds.has(matchedGame.id)) {
            console.warn(`⚠️ Game "${gameName}" already matched (duplicate ID: ${matchedGame.id})`);
            missingGames.push(gameName);
          } else {
