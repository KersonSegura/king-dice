  ...
  try {
    ...
    if (availableGames.length === 0) {
      return NextResponse.json({ 
        error: 'No games found for Card Mode' 
      }, { status: 500 });
    }
    ...
    const getDailyGameIndex = (): number => {
      ...
      // Use a completely different seed for Card Mode - separate from Title and Image Modes
      const seed = dayOfYear + 10000;
      const randomIndex = seededRandom(seed, availableGames.length);
      
      // Log only the day and index, not the game name
      console.log(`📅 Card Mode: Day ${dayOfYear}, random game index ${randomIndex + 1}`);
    ...
    const dailyGame = availableGames[dailyGameIndex];
    
    
    if (!dailyGame) {
      return NextResponse.json({ 
        error: 'Failed to get daily game for Card Mode' 
      }, { status: 500 });
    }
    ...
    const fetchCluesFromDatabase = async (gameName: string): Promise<string[]> => {
      try {
        const response = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/boardle/hints?gameName=${encodeURIComponent(gameName)}&gameMode=card`);
        if (response.ok) {
      ...
      return [
        'Players use cards to make moves or take actions.',
        'The game involves collecting or playing cards strategically.',
        'Players compete to score points or achieve goals.',
        'Cards have different values, effects, or abilities.',
        'Hand management and timing are important for success.'
      ];
    };
    ...
    const boardleGames = [{
      ...
      imageUrl: `/api/boardle/card-image?id=${dailyGameIndex}`,
      clues: clues,
      zoomType: dailyGame.zoomType
    }];
    
    console.log(`🎯 Card Mode: Daily game selected (Day ${dailyGameIndex + 1})`);
    
    return NextResponse.json({
      games: boardleGames,
      total: boardleGames.length,
      message: `Daily rotation: Game ${dailyGameIndex + 1} for Card Mode`
    });
    
  } catch (error) {
    console.error('Error fetching Card Mode games:', error);
    return NextResponse.json({ 
      error: 'Failed to fetch Card Mode games',
