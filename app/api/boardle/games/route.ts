...
export async function GET(request: NextRequest) {
  try {
    ...
    // Read the games list file - dedicated Title Mode list
    const gamesListPath = path.join(process.cwd(), 'title-mode-games-list.txt');
    const gamesListContent = await fs.readFile(gamesListPath, 'utf-8');
    
    // Parse the games list - Title Mode extracts game names from numbered format
    const titleGames = gamesListContent
      .split('\n')
      ...
      .map((line: string) => {
        ...
        // Extract game name by removing numeric prefix (e.g., "020. Pioneer Days" → "Pioneer Days")
        // Pattern: optional number, optional period, optional space, then the actual game name
        const gameNameMatch = trimmedLine.match(/^\d+\.?\s*(.+)$/);
        const gameName = gameNameMatch ? gameNameMatch[1].trim() : trimmedLine;
        
        return { name: gameName };
      })
      .filter((game): game is { name: string } => game !== null);
    
    if (titleGames.length === 0) {
      return NextResponse.json({ 
        error: 'No games found for Title Mode' 
      }, { status: 500 });
    }
    ...
    const getDailyGameIndex = (): number => {
      ...
      // Use a completely different seed for Title Mode - separate from Image and Card Modes
      const seed = dayOfYear + 1000;
      const randomIndex = seededRandom(seed, titleGames.length);
      
      // Log only the day and index, not the game name
      console.log(`📅 Title Mode: Day ${dayOfYear}, random game index ${randomIndex + 1}`);
      
      return randomIndex;
    };

    // Get today's daily game
    const dailyGameIndex = getDailyGameIndex();
    const dailyGame = titleGames[dailyGameIndex];
    
    
    if (!dailyGame) {
      return NextResponse.json({ 
        error: 'Failed to get daily game for Title Mode' 
      }, { status: 500 });
    }
    ...
    const generateCluesForGame = (gameName: string): string[] => {
      ...
      if (lowerName.includes('catan')) {
        ...
        // Generate clues based on game name characteristics
        if (lowerName.includes('war') || lowerName.includes('battle') || lowerName.includes('conflict')) {
          ...
          if (lowerName.includes('war') || lowerName.includes('battle') || lowerName.includes('conflict')) {
            ...
            clues.push('This game typically plays in 30-90 minutes.');
            clues.push('This game supports 2-6 players.');
            clues.push('This game involves strategic thinking and player interaction.');
          }
        }
      ...
      // Default return if no specific clues were generated
      return clues.slice(0, 5);
    };
    ...
    const fetchCluesFromDatabase = async (gameName: string): Promise<string[]> => {
      try {
        const response = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/boardle/hints?gameName=${encodeURIComponent(gameName)}&gameMode=title`);
        if (response.ok) {
      ...
      return [
        'Players take turns making moves on a game board.',
        'The game involves collecting resources or scoring points.',
        'Players compete to achieve victory conditions.',
        'The game includes cards, dice, or other game components.',
        'Strategy and planning are important for success.'
      ];
    };
    ...
    const boardleGames = [{
      ...
      imageUrl: '', // Title Mode doesn't use images
      clues: clues
    }];
    
    console.log(`🎯 Title Mode: Daily game selected (Day ${dailyGameIndex + 1})`);
    
    const response = NextResponse.json({
      games: boardleGames,
      total: boardleGames.length,
      message: `Daily rotation: Game ${dailyGameIndex + 1} for Title Mode`,
    ...
    return NextResponse.json({ 
      error: 'Failed to fetch Title Mode games',
