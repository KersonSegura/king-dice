...
export default function AdminPage() {
  ...
  const verifyAccess = async () => {
    ...
    try {
      ...
      setVerified(true);
      await Promise.all([loadDashboard(), loadUsers('', false)]);
    } catch (err) {
  ...
  const loadUsers = async (q: string, blocked?: boolean) => {
    const blockedParam = blocked ?? blockedOnly ? '1' : '0';
    const data = await requestJson<{ success: boolean; users: AdminUser[] }>(
      `/api/admin/users?q=${encodeURIComponent(q)}&limit=50&blocked=${blockedParam}`
    );
    setUsers(data.users || []);
  };
  ...
  const onSearch = async () => {
    try {
      setError('');
      await loadUsers(query, blockedOnly);
    } catch (err) {
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={() => {
                ...
                loadUsers(query, blockedOnly).catch((err) => setError(err instanceof Error ? err.message : 'Refresh failed'));
              }}
            ...
            <button
              type="button"
              onClick={exitAdmin}
              className="rounded-lg border border-white/20 px-3 py-2 text-sm hover:bg-white/10 text-white/80"
            >
              Exit Admin
            </button>
          </div>
        ...
        {dashboard && (
          <>
            <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <p className="text-xs text-white/60">New Users (30d)</p>
                <p className="text-2xl font-bold">{dashboard.metrics.newUsers30d}</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <p className="text-xs text-white/60">Pending Reports</p>
                <p className="text-2xl font-bold">{dashboard.metrics.pendingReports ?? 0}</p>
          ...
          <div className="flex flex-col md:flex-row gap-2 md:items-center md:justify-between mb-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <label className="flex items-center gap-2 text-sm text-white/80 cursor-pointer">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    loadUsers(query, e.target.checked);
                  }}
              ...
              <input
                ...
                className="rounded-lg border border-white/20 bg-black/30 px-3 py-2 text-sm min-w-[200px]"
              />
              <button
                type="button"
                onClick={onSearch}
